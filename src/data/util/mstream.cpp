...
namespace fenix::data::util::detail {
...
OMmapStreamBuf::OMmapStreamBuf() : std::streambuf() {
...
OMmapStreamBuf::~OMmapStreamBuf() {
  if (mmap_address) munmap(mmap_address, claim_len);
}

size_t OMmapStreamBuf::written_len() {
...
int_type OMmapStreamBuf::overflow(int_type ch) {
...
pos_type OMmapStreamBuf::seekoff(
...
// --------------------
// MStreamBuf functions
// --------------------

MStreamBuf::MStreamBuf(char* b, size_t l) : std::streambuf() {
  buf = b;
  len = l;
  setp(buf, buf + len);
...
int_type MStreamBuf::overflow(int_type ch) {
...
pos_type MStreamBuf::seekpos(pos_type pos, std::ios_base::openmode which) {
  if (which & std::ios_base::out) setp(buf + pos, buf + len);
  if (which & std::ios_base::in) setg(buf, buf + pos, buf + len);
  return pos;
}

pos_type MStreamBuf::seekoff(
  off_type off, std::ios_base::seekdir dir, std::ios_base::openmode which
) {
