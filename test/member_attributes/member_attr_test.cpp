...
int main(int argc, char** argv) {
  ...
  //===========================================================================
  if (rank == 0) fprintf(stderr, "Test 1: Reduce count from 100 to 50 before staging\n");
  ...
  if (rank == 0) fprintf(stderr, "  Loading checkpoint, expecting only 50 elements restored\n");
  member_restore(my_group, 1, FENIX_DATA_RESTORE_INPLACE,
                 FENIX_DATA_RESTORE_FULL, FENIX_DATA_SNAPSHOT_LATEST);

  // Verify first 50 elements were restored correctly
  for (int i = 0; i < 50; i++) {
    if (data[i] != rank * 1000 + i) {
      fprintf(stderr, "Rank %d: ERROR in element %d, expected %d, got %d\n",
              rank, i, rank * 1000 + i, data[i]);
  ...
  for (int i = 50; i < 100; i++) {
    if (data[i] != -999) {
      fprintf(stderr, "Rank %d: ERROR - element %d was restored but shouldn't have been (got %d)\n",
              rank, i, data[i]);
      MPI_Abort(res_comm, 1);
    }
  }
  ...
  //===========================================================================
  if (rank == 0) fprintf(stderr, "Test 2: Change from MPI_2INT to MPI_INT before staging\n");
  ...
  member_attr_set(my_group, 2, FENIX_DATA_MEMBER_ATTRIBUTE_DATATYPE, &new_dtype, &flag);
  ...
  if (rank == 0) fprintf(stderr, "  Loading checkpoint, expecting 50 MPI_INTs restored\n");
  member_restore(my_group, 2, FENIX_DATA_RESTORE_INPLACE,
                 FENIX_DATA_RESTORE_FULL, FENIX_DATA_SNAPSHOT_LATEST);
  ...
  for (int i = 0; i < 50; i++) {
    if (pair_data[i] != rank * 1000 + i) {
      fprintf(stderr, "Rank %d: ERROR in element %d, expected %d, got %d\n",
              rank, i, rank * 1000 + i, pair_data[i]);
      MPI_Abort(res_comm, 1);
    }
  }
  ...
  for (int i = 50; i < 100; i++) {
    if (pair_data[i] != -888) {
      fprintf(stderr, "Rank %d: ERROR - element %d was restored but shouldn't (got %d)\n",
              rank, i, pair_data[i]);
      MPI_Abort(res_comm, 1);
    }
  }
  ...
  //===========================================================================
  if (rank == 0) fprintf(stderr, "Test 3: Verify buffer pointer can change after staging\n");
  ...
  member_attr_set(my_group, 3, FENIX_DATA_MEMBER_ATTRIBUTE_BUFFER, buf2.data(), &flag);
  ...
  member_restore(my_group, 3, FENIX_DATA_RESTORE_INPLACE,
                 FENIX_DATA_RESTORE_FULL, FENIX_DATA_SNAPSHOT_LATEST);

  // Verify buf2 has the data from the second checkpoint
  for (int i = 0; i < 50; i++) {
    if (buf2[i] != rank * 200 + i) {
      fprintf(stderr, "Rank %d: ERROR after buffer change, element %d = %d\n",
              rank, i, buf2[i]);
      MPI_Abort(res_comm, 1);
    }
  }

  if (rank == 0) fprintf(stderr, "  ✓ Buffer pointer change worked correctly\n");
  ...
  //===========================================================================
  if (rank == 0) fprintf(stderr, "Test 4: Verify count change to different value rejected after staging\n");
  ...
  try {
    member_attr_set(my_group, 4, FENIX_DATA_MEMBER_ATTRIBUTE_COUNT, &new_count, &flag);
  } catch (const fenix::RuntimeException& e) {
    caught_exception = true;
  }

  if (!caught_exception) {
    if (rank == 0) fprintf(stderr, "  ✗ ERROR: Count change should have thrown!\n");
    MPI_Abort(res_comm, 1);
  }
  ...
  try {
    ...
    if (rank == 0) fprintf(stderr, "  ✗ ERROR: Setting count to same value should be allowed!\n");
    MPI_Abort(res_comm, 1);
  }
  ...
  //===========================================================================
  if (rank == 0) fprintf(stderr, "Test 5: Verify datatype change to different value rejected after staging\n");

  member_create(my_group, 5, test_data.data(), 100, MPI_INT);
  checkpoint(my_group, SUBSET_FULL);

  // Attempt to change datatype to a DIFFERENT value after staging - should throw
  MPI_Datatype dtype_double = MPI_DOUBLE;
  caught_exception = false;
  try {
    member_attr_set(my_group, 5, FENIX_DATA_MEMBER_ATTRIBUTE_DATATYPE, &dtype_double, &flag);
  } catch (const fenix::RuntimeException& e) {
    caught_exception = true;
  }

  if (!caught_exception) {
    if (rank == 0) fprintf(stderr, "  ✗ ERROR: Datatype change should have thrown!\n");
    MPI_Abort(res_comm, 1);
  }
  ...
  try {
    member_attr_set(my_group, 5, FENIX_DATA_MEMBER_ATTRIBUTE_DATATYPE, &dtype_int, &flag);
    if (rank == 0) fprintf(stderr, "  ✓ Setting datatype to same value allowed\n");
  } catch (const fenix::RuntimeException& e) {
    if (rank == 0) fprintf(stderr, "  ✗ ERROR: Setting datatype to same value should be allowed!\n");
    MPI_Abort(res_comm, 1);
  }
