...
int main(int argc, char **argv) {
  ...
  int error;
  int spare_ranks = 1;
  Fenix_Init(&fenix_status, MPI_COMM_WORLD, &newcomm, &argc, &argv, spare_ranks,
             &error);
#else
#warning "no fenix"
  ...
  for (it = 0; it < its; it++) {
    if (it == 2)
      tstart = MPI_Wtime();
    // for(i=0 ; i<isends ; i++)
    //     MPI_Isend(&(bufs[i]), 1, MPI_INT, (rank+1)%size, 0, newcomm,
    //     &(reqs[i]));
    for (i = 0; i < isends; i++) {
    ...
    for (i = 0; i < isends; i++)
      MPI_Irecv(&(bufs_recv[i]), 1, MPI_INT, (rank - 1) % size, 0, newcomm,
                &(reqs_recv[i]));
