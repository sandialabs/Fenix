...
int main(int argc, char** argv) {
  ...
  if (rank == 0) fprintf(stderr, "  Loading latest checkpoint (timestamp 1) into target buffer\n");
  std::vector<int> target_buffer(100, -1);
  fenix::DataSubset found_subset;

  int ret = member_load(my_group, my_member, target_buffer.data(), 100, 1, found_subset);
  ...
  for (int i = 0; i < 100; i++) {
    if (target_buffer[i] != rank * 2000 + i) {
      fprintf(stderr, "Rank %d: ERROR in target buffer[%d], got %d, expected %d\n",
  ...
  if (rank == 0) fprintf(stderr, "  Loading first checkpoint (timestamp 0) into target buffer\n");
  std::fill(target_buffer.begin(), target_buffer.end(), -1);

  ret = member_load(my_group, my_member, target_buffer.data(), 100, 0, found_subset);
  if (ret != FENIX_SUCCESS) {
  ...
  for (int i = 0; i < 100; i++) {
    if (target_buffer[i] != rank * 1000 + i) {
      fprintf(stderr, "Rank %d: ERROR loading timestamp 0 at [%d], got %d, expected %d\n",
  ...
  for (int i = 0; i < 100; i++) {
    if (data[i] != rank * 2000 + i) {
      fprintf(stderr, "Rank %d: ERROR - original data was modified at [%d]\n", rank, i);
      MPI_Abort(res_comm, 1);
    }
  }
  ...
  if (rank == 0) fprintf(stderr, "All member_load target buffer tests passed!\n");
