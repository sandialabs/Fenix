...
int main(int argc, char** argv) {
  ...
  if (rank == 0) fprintf(stderr, "Test: member_load with smaller target buffer\n");
  ...
  if (rank == 0) fprintf(stderr, "  Loading first 50 elements into smaller buffer\n");
  std::vector<int> target_buffer(50, -999);
  fenix::DataSubset found_subset;

  int ret = member_load(my_group, my_member, target_buffer.data(), 50, 0, found_subset);
  if (ret != FENIX_SUCCESS) {
    fprintf(stderr, "Rank %d: ERROR loading into smaller buffer, ret=%d\n", rank, ret);
  ...
  for (int i = 0; i < 50; i++) {
    if (target_buffer[i] != rank * 1000 + i) {
      fprintf(stderr, "Rank %d: ERROR at [%d], got %d, expected %d\n",
  ...
  // Test loading into a larger buffer (should succeed with partial restore warning)
  if (rank == 0) fprintf(stderr, "  Loading into larger buffer (150 elements, checkpoint has 100)\n");
  std::vector<int> large_buffer(150, -888);

  ret = member_load(my_group, my_member, large_buffer.data(), 150, 0, found_subset);

  // Should get FENIX_WARNING_PARTIAL_RESTORE (101) since checkpoint only has 100 elements
  if (ret != 101) {
    fprintf(stderr, "Rank %d: ERROR expected partial restore warning (101), got %d\n", rank, ret);
    MPI_Abort(res_comm, 1);
  }
  ...
  for (int i = 0; i < 100; i++) {
    if (large_buffer[i] != rank * 1000 + i) {
      fprintf(stderr, "Rank %d: ERROR at [%d], got %d, expected %d\n",
              rank, i, large_buffer[i], rank * 1000 + i);
      MPI_Abort(res_comm, 1);
    }
  }
  ...
  for (int i = 100; i < 150; i++) {
    if (large_buffer[i] != -888) {
      fprintf(stderr, "Rank %d: ERROR - element [%d] was modified (got %d)\n",
              rank, i, large_buffer[i]);
      MPI_Abort(res_comm, 1);
    }
  }

  if (rank == 0) fprintf(stderr, "  ✓ Partial restore warning correctly returned\n");
