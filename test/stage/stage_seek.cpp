...
int main(int argc, char** argv) {
  ...
  fenix::data::util::detail::OMmapStreamBuf::target_claim_chunk_size =
    40 * 1024; // 40KB
  fenix::data::util::detail::OMmapStreamBuf::target_write_chunk_size =
    4 * 1024; // 4KB

  group_create(my_group, {.depth = 1});

  if (rank == 0)
    fprintf(stderr, "Test: Seeking and writing past chunk boundaries\n");
  ...
  // Create resizeable member with iostream serializer that uses extensive
  // seeking
  member_create(
    ...
    [&](std::iostream& strm, int dir, void* b, int offset, int count) {
      ...
      if (dir == FENIX_SERIALIZE) {
        ...
        if (rank == 0)
          fprintf(
            stderr, "Writing %d ints (%zu bytes) with seeking\n", size,
            size * sizeof(int)
          );
        ...
        strm.seekp(pos1);
        strm.write(
          (char*)(data.data() + quarter), sizeof(int) * (size - quarter)
        );
        auto pos4 = strm.tellp();
        if (rank == 0)
          fprintf(
            stderr, "After writing remaining data: pos=%ld\n", (long)pos4
          );
        ...
        if (rank == 0)
          fprintf(stderr, "After seekp(0, end): pos=%ld\n", (long)pos5);
  ...
  if (rank == 0)
    fprintf(stderr, "Staging and committing with seeking serializer\n");
  ...
  if (rank == 0)
    fprintf(stderr, "Restoring data to verify seeking worked correctly\n");
  ...
  if (data.size() != original_data.size()) {
    fprintf(
      stderr, "Rank %d: Size mismatch! Expected %zu, got %zu\n", rank,
  ...
  for (size_t i = 0; i < data.size(); i++) {
    if (data[i] != original_data[i]) {
      fprintf(
        stderr, "Rank %d: Data mismatch at index %zu! Expected %d, got %d\n",
  ...
  if (rank == 0)
    fprintf(
      stderr,
      "Successfully wrote past first write_chunk and skipped multiple chunks\n"
    );
